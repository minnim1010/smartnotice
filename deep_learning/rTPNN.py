...
class rTPNN:
    ...
    def SDPs(self):
        ...
        for i in range(self.num_sensors):
            # Input1 is x_{k} and Input2 is x_{k-1}
            inputs_SDP.append(Input(shape=(2, 1, )))
            x_k = Lambda(self.slice_0, output_shape=self.slice_output_shape)(
                inputs_SDP[-1])
            x_k1 = Lambda(self.slice_1, output_shape=self.slice_output_shape)(
                inputs_SDP[-1])

            # Trend Predictor
            trend_predictor.append(Subtract()([x_k, x_k1]))
            trend_predictor[-1] = SimpleRNN(1, activation=None,
                                            use_bias=False)(trend_predictor[-1])

            # Level Predictor
            level_predictor.append(x_k)
            level_predictor[-1] = SimpleRNN(1, activation=None,
                                            use_bias=False)(level_predictor[-1])

            # SDP Dense Neuron
            outputs_SDP.append(Concatenate()(
                [trend_predictor[-1], level_predictor[-1], Reshape((1,))(x_k)]))
    ...
    def create_model(self):
        ...
        output = Dense(self.predictor_arch[0], activation=self.activation_name)(
            input_fully_connected)
