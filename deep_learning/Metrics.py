...
class Metrics:
    ...
    def calc_conf_mat(self, actual, predicted):
        ...
        true_positive = np.sum((actual == predicted)[
                               positive_indices]) / positive_indices.shape[0]
        true_negative = np.sum((actual == predicted)[
                               negative_indices]) / negative_indices.shape[0]
    ...
    # calculate alarming time: return the fastest fire detection time
    def calc_alarming_time(self, actual, predicted, relative_time):
        ...
        try:
            predicted_fire_first_index = np.where(predicted == 1)[0]
        except:
            # alarming time will be negative value
            predicted_fire_first_index = predicted.shape[0]-1
        alarming_time = min(relative_time[predicted_fire_first_index] - 10)
        # print(alarming_time)
